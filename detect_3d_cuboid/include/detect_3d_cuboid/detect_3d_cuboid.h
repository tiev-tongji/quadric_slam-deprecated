...
class cuboid  // matlab cuboid struct. cuboid on ground. only has yaw, no obj
              // roll/pitch
{
 public:
  Eigen::Vector3d pos;
...
typedef std::vector<cuboid*>
    ObjectSet;  // for each 2D box, the set of generated 3D cuboids
...
class detect_3d_cuboid {
 public:
  ...
  // object detector needs image, camera pose, and 2D bounding boxes(n*5, each
  // row: xywh+prob)  long edges: n*4.  all number start from 0
  ...
  bool whether_sample_cam_roll_pitch =
      false;  // sample camera roll pitch in case don't have good camera pose
  bool whether_sample_bbox_height =
      false;  // sample object height as raw detection might not be accurate
